...
private void buscarLibroPorTitulo() {
    ...
    if (json.isEmpty() || !json.contains("\"count\":0,\"next\":null,\"previous\":null,\"results\":[]")) {
        ...
        if (libroBuscado.isPresent()) {
            ...
            try {
                List<Libros> libroEncontrado = libroBuscado.stream()
                        .map(a -> new Libros(a))
                ...
                Optional<Libros> libroOptional = repository.buscarLibroPorNombre(titulo);
...
private void listarLibrosRegistrados() {
    List<Libros> libros = repository.librosRegistrados();
...
private void buscarLibrosPorIdioma(String idioma) {
    try {
        ...
        List<Libros> libros = repository.librosPorIdioma(idiomaEnum);
...
private void listarAutorPorFechaFallecimiento(){
    System.out.println("Ingrese el año de muerte del autor");
    try {
        var muerte = Integer.valueOf(teclado.nextLine());
        List<Autor> autor = repository.listarAutorPorFechaDeMuerte(muerte);
        if (!autor.isEmpty()) {
            autor.stream()
                    .forEach(System.out::println);
        } else {
            System.out.println("No se encontró ningún actor muerto en este año");
        }
    } catch (NumberFormatException e) {
        System.out.println("Ingrese un año válido. - Warning: " + e.getMessage());
    }
